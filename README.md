...
- Optional request-bound sudo approval verifier -- a separate, sessionless, UV-required WebAuthn surface for one exact short-lived command/requester/expiry binding. It has its own state and trusted local enrollment/consumer path; see [docs/sudo-approval.md](docs/sudo-approval.md)
