...
- Optional request-bound sudo approval verifier -- a separate verifier-only service/UID, sessionless UV-required WebAuthn surface, isolated mounts, and dedicated broker API for one exact canonical argv/cwd/requester/broker/expiry transaction. The broad WebUI has no verifier routes or credentials; see [docs/sudo-approval.md](docs/sudo-approval.md)
