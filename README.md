...
- Optional request-bound sudo approval verifier -- a separate, sessionless, UV-required WebAuthn surface for one exact canonical argv/cwd/requester/broker/expiry transaction. It has isolated durable state, local-only credential administration, and a dedicated broker-authenticated create/status/consume API; see [docs/sudo-approval.md](docs/sudo-approval.md)
