...
from api.routes import _is_verifier_only_path, apply_cors_preflight_headers, handle_delete, handle_get, handle_patch, handle_post, handle_put
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if _is_verifier_only_path(parsed.path): return j(self, {"error": "not found"}, status=404)
    ...
    def do_OPTIONS(self) -> None:
        ...
        self._req_t0 = time.time()
        if _is_verifier_only_path(urlparse(getattr(self, "path", "/")).path): return j(self, {"error": "not found"}, status=404)
        self.send_response(200)
        apply_cors_preflight_headers(self)
        # Frame the empty preflight; otherwise HTTP/1.1 keep-alive reads until close.
